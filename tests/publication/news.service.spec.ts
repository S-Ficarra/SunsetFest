...
describe('NewsService', () => {
  ...
  beforeEach(() => {
    ...
    newsService = new NewsService(newsRepository, roleService);
    ...
    expect(news).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          _type: 'news',
          _content: expect.objectContaining({ _title: 'title1' }),
        }),
        expect.objectContaining({
          _type: 'news',
          _content: expect.objectContaining({ _title: 'title2' }),
        }),
      ]),
    );
  });
  ...
  it('should return a news just created', async () => {
    const foundNews3 = new News(
      userRepository.users[0],
      new Date(),
      new Date(),
      true,
      new Content('title3', 'text3', 'imageURL'),
    );
    await newsService.createNews(foundNews3);
    expect(foundNews3).toEqual(
      expect.objectContaining({
        _id: 3,
        _type: 'news',
        _content: expect.objectContaining({ _title: 'title3' }),
      }),
    );
  });
  ...
  it('should return a news with titleEdited and textEdited', async () => {
    ...
    expect(foundNewsEdited).toEqual(
      expect.objectContaining({
  ...
  it('should delete the publication with the id 1', async () => {
    const deleteNewsCall = async () =>
      await newsService.deleteNews(userRepository.users[0], 1);
