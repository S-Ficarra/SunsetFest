...
describe('ToiletService', () => {
    ...
    //getAllToilets
    it('Should return all toilets', () => {
        const toilets = toiletService.getAllToilets();
    ...
    //getToiletById
    it('Should return the toilet id 1 with the question: question 1', () => {
        const foundToilet1 = toiletService.getToiletById(1);
        expect(foundToilet1).toEqual(expect.objectContaining({_name: 'danger'}));
    });


    //createToilet
    it('should return the new toilet created', () => {
        let foundToilet3 = new Toilet ('coffin', 987.459, 415.596);
        toiletService.createToilet(foundToilet3);
        expect(foundToilet3).toEqual(expect.objectContaining({_name: 'coffin'}));
    });


    //editToilet
    it('should return the toilet1 with question and answer edited', () => {
        let editedToilet = new Toilet ('coffin', 125.366, 125.258);
        editedToilet.setId(1);
        toiletService.editToilet(editedToilet);
        let foundToiletEdited = toiletService.getToiletById(1);
        expect(foundToiletEdited).toEqual(expect.objectContaining({_name: 'coffin'}));        
    });


    //deleteToilet
    it('should return the toilets array without the toilet with id 1', () => {
